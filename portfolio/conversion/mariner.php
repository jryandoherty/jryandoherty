<?php include "../header.php"; ?>

<div class="container webpage">
  <div class="row">
    <div class="col-md-4"></div>
    <div class="col-md-4 col-md-offset-4 crumbs">
      <?php include "../breadcrumbs.php"; ?>
    </div>
  </div> <!--end breadcrumb row -->
  <div class="row">
    <div id="carousel-port" class="carousel slide" data-ride="carousel" data-interval="3000">
      <!-- Wrapper for slides -->
      <div class="carousel-inner" role="listbox">
        <div class="item active">
          <div class="col-lg-12"><img src="/portfolio/2016/mariner/screenshot-2.png" alt="image 1"></div>
        </div>

        <div class="item">
          <div class="col-lg-12"><img src="/portfolio/2016/mariner/screenshot-3.png" alt="image 2"></div>
        </div>

        <div class="item">
          <div class="col-lg-12"><img src="/portfolio/2016/mariner/screenshot-1.png" alt="image 3"></div>
        </div>

        <ol class="carousel-indicators">
          <li data-target="#carousel-port" data-slide-to="0" class="active"></li>
          <li data-target="#carousel-port" data-slide-to="1"></li>
          <li data-target="#carousel-port" data-slide-to="2"></li>
        </ol>
      </div>

    <div class="col-lg-12">Conversion pages for Buyerzone.com.</div>

    </div><!-- end carousel -->

    <div class="legend">
      <h2>PROJECT</h2>
      <ul>
        <li><strong>Category:</strong> Conversion pages</li>
        <li><strong>Client:</strong> Buyerzone.com</li>
        <li><strong>Date:</strong> 2015</li>
        https://www.buyerzone.com/office-equipment/digital-copiers/rfqt
      </ul>
    </div>

  </div><!-- end rowr -->
</div><!-- end container -->
<?php include "../footer.php"; ?>
