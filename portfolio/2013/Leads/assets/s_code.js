...
/* dev account */
var s_account="tmn-dev-buyerzone-ttr,tmn-dev-global";

/* production account */
//var s_account="tmn-prod-buyerzone,ttrglobalproduction";
...
s.linkInternalFilters="javascript:,toptenreviews.com,hermanstreet.com,techmedianetwork.com,laptopmag.com,space.com,livescience.com,technewsdaily.com,newsarama.com,lifeslittlemysteries.com,ouramazingplanet.com,ipadnewsdaily.com,businessnewsdaily.com,myhelthnewsdaily.com,securitynewsdaily.com,innovationnewsdaily.com,ittechnewsdaily.com,northorion.com,buyerzone.com"
...
function s_doPlugins(s) {
	...
	s.clickPast(s.SEMvar,'event99','event100');
	...
	//single parameter
	if(!s.campaign)
		s.campaign=s.getValOnce(s.getQueryParam('cmpid'),'s_campaign',0);

	if(!s.eVar13)
		s.eVar13=s.getQueryParam('intcmp');

	if(!s.eVar19)
		s.eVar19=s.getQueryParam('a_aid');

	if (!s.eVar22)
		s.eVar22=s.getQueryParam('a_sid');

	if(s.prop7)
		s.events = s.apl(s.events,"event1",",",2);
	s.prop8=s.getTimeParting('h','-7','2009'); // Set hour
	s.prop9=s.getTimeParting('d','-7','2009'); // Set day
	s.prop10=s.getTimeParting('w','-7','2009'); // Set Weekend / Weekday
	s.prop26="TechMediaNetwork";
	s.prop27="BuyerZone";

	if(!s.eVar1 && s.prop1) s.eVar1 = s.prop1;
	if(!s.eVar2 && s.prop2) s.eVar2 = s.prop2;
	if(!s.eVar3 && s.prop3) s.eVar3 = s.prop3;
	if(!s.eVar4 && s.prop4) s.eVar4 = s.prop4;
	if(!s.eVar5 && s.prop5) s.eVar5 = s.prop5;
	if(!s.eVar6 && s.prop6) s.eVar6 = s.prop6;
	if(!s.eVar7 && s.prop7) s.eVar7 = s.prop7;
	if(!s.eVar8 && s.prop8) s.eVar8 = s.prop8;
	if(!s.eVar9 && s.prop9) s.eVar9 = s.prop9;
	if(!s.eVar10 && s.prop10) s.eVar10 = s.prop10;
	if(!s.eVar11 && s.pageName) s.eVar11 = s.pageName;
	if(!s.eVar12 && s.channel) s.eVar12 = s.channel;
	if(!s.eVar19 && s.prop19) s.eVar19 = s.prop19;
	if(!s.eVar22 && s.prop22) s.eVar22 = s.prop22;
	if(!s.eVar36 && s.prop26) s.eVar36 = s.prop26;
	if(!s.eVar37 && s.prop27) s.eVar37 = s.prop27;
	if(!s.eVar42 && s.prop29) s.eVar42 = s.prop29;
	...
	// Google keyword postion tracking - this probably needs to be refactored at some point
	// Google says that only google.com/url will pass the result position:
	// http://analytics.blogspot.com/2009/04/upcoming-change-to-googlecom-search.html
	if (document.referrer.indexOf('https://web.archive.org/web/20140416052243/http://www.google.com/url') == 0) {
		var organic = false;
		var position = false;
		var source = false;
		...
		for (var i = 0; i < params.length; i++) {
			...
			if (parts[0] == 'ct' && parts[1] == 'res') {
				organic = true;
			}

			if (parts[0] == 'source' && parts[1] == 'web') {
				source = true;
			}
		}

		// Need to make sure all three of these conditions are true to be sure it is an organic google result
		if (organic && position && source) {
			s.prop21 = s.eVar21 = position;
		}
	}
}
...
s.visitorNamespace="toptenreviews"
s.trackingServer="metrics.toptenreviews.com" //make sure this value matches your 1st party cookie non-ssl domain
s.trackingServerSecure="smetrics.toptenreviews.com"  //make sure this value matches your 1st party cookie ssl domain
s.dc="122"
...
/*
     FILE ARCHIVED ON 05:22:43 Apr 16, 2014 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 00:47:25 Aug 08, 2019.
...
/*
playback timings (ms):
  LoadShardBlock: 981.279 (3)
  esindex: 0.017
  captures_list: 1049.251
  CDXLines.iter: 25.694 (3)
  PetaboxLoader3.datanode: 1023.634 (4)
  exclusion.robots: 0.238
  exclusion.robots.policy: 0.222
  RedisCDXSource: 34.272
  PetaboxLoader3.resolve: 36.697
  load_resource: 217.685
*/